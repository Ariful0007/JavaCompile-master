...
public class ProjectFragment extends BaseFragment<ProjectContract.Presenter> implements ProjectContract.View, MenuItem.OnMenuItemClickListener {
    ...
    public boolean onMenuItemClick(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case 2:
                ...
                MyUtils.getAlert(activity, "Only zip-format source packages exported by the software are supported for import", (dialog, which) -> {
    ...
    public void showOperateMenu(AppCompatImageView moreView, File dir) {
        ...
        if (StringUtils.equals(filePath, javaProject.getLibDirPath())) {
            popupMenu.getMenu().add("Import Maven dependencies").setOnMenuItemClickListener(item -> {
                //showNewProjectDialog();
                return true;
            });
            popupMenu.getMenu().add("Local Jar dependency").setOnMenuItemClickListener(item -> {
                Intent intent = new Intent(Intent.ACTION_GET_CONTENT);
        ...
        // src 文件夹及其子文件夹菜单
            ...
            popupMenu.getMenu().add("New Java Package").setOnMenuItemClickListener(item -> {
                showCreatePackageDialog(dir);
                return true;
            });
            popupMenu.getMenu().add("New Java Class").setOnMenuItemClickListener(item -> {
        ...
        // 工程文件夹
            ...
            popupMenu.getMenu().add("Close the project").setOnMenuItemClickListener(item -> {
                // 关闭项目
                projectContainer.setVisibility(View.GONE);
                projectRecycler.setVisibility(View.VISIBLE);
        ...
        AlertDialog alertDialog = new AlertDialog.Builder(activity)
                ...
                .setPositiveButton(R.string.dialog_done, (dialog, which) -> {
                    ...
                    if (StringUtils.isEmpty(className)) {
                        ToastUtils.showShort("Please enter the class name");
                        return;
                    }
                    ...
                    if (methodCheckBox.isChecked()) {
                        ...
                        ToastUtils.showShort(newJavaFile.getName() + " Create the result：" + b);
                    } else {
